  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-semibold">{t.trendingTopics.title}</h3>
      </div>
      {sorted.length === 0 && (
        <p className="text-xs text-muted-foreground mb-3">{t.trendingTopics.empty}</p>
      )}
        ...
        {topics.map(({ category, count }, i) => (
          <Link key={category} to={`/articles?category=${category}`}>
            <div className="flex items-center justify-between p-2.5 rounded-xl hover:bg-muted/50 transition-colors group">
              ...
              {count > 0 && (
                <Badge variant="secondary" className="text-xs rounded-full">{count} {t.trendingTopics.savedSuffix}</Badge>
              )}
            </div>
          </Link>
        ))}
